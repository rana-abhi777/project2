...
class NewViewController: UIViewController , IndicatorInfoProvider,NewProductTask {
    ...
    func configureCollectionView(){
        collectionViewdataSource = CollectionViewDataSource(items: arrProduct, collectionView: collectionViewNewProducts, cellIdentifier: CellIdentifiers.NewCollectionViewCell.rawValue, headerIdentifier: "", cellHeight: 275, cellWidth: (collectionViewNewProducts.frame.size.width - 8)/2, cellSpacing: 8, configureCellBlock: {[unowned self] (cell, item, indexpath) in
            let cell = cell as? NewCollectionViewCell
    ...
    func hitApiForPopularProduct() {
        ApiManager().getDataOfURL(withApi: API.GetNewProduct(APIParameters.GetNewProduct(pageNo : pageNo).formatParameters()), failure: { (err) in
            print(err)
