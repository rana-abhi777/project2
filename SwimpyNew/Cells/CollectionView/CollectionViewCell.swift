...
final class CollectionViewCell: UICollectionViewCell {
    ...
    var category : Category?{
        didSet{
            updateUI(model: category!)
        }
    }
    
    
    
    private func updateUI(model : Category){

        defer{
            titleLabel?.text = model.name
        }
        overlayView?.backgroundColor = UIColor().getRandomColor()

        guard let url = model.categoryImageOriginal   else {
            return }
